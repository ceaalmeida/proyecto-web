class ProveedoresAlimentosService {
    async getAllProveedoresAlimentos(token) {
      const proveedores = await fetch("http://localhost:3000/proveedores-alimentos/", {
        method: "GET",
        headers: {
    ...
    async updateProveedorAlimentos(id, proveedor) {
      const proveedorResponse = await fetch(`http://localhost:3000/proveedores-alimentos/${id}`, {
        method: "PUT",
    ...
    async createProveedorAlimentos(proveedor) {
      const proveedorResponse = await fetch("http://localhost:3000/proveedores-alimentos/", {     
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(proveedor),
      });
    ...
    async deleteProveedorAlimentos(id) {
      await fetch(`http://localhost:3000/proveedores-alimentos/${id}`, {
        method: "DELETE",
      });
    }
  }
