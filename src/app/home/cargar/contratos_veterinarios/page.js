...
export default function TableContratosVeterinarios() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filtrar solo los contratos con "Nombre" igual a "Contrato Veterinario"
        const contratosVeterinarios = response.filter(
          (contrato) => contrato.Nombre === "Contrato Veterinario"
        );

        setData(contratosVeterinarios);
      } catch (error) {
  ...
  const exportarPDF = () => {
    ...
    const marginLeft = 40;
    const title = `Reporte de Contratos Veterinarios`;
    ...
    doc.save(`contratos_veterinarios_report.pdf`);
    setExportando(false);
  };
